...
test('optimized WebP assets meet source budgets', () => {
  ...
  assert.ok(
    webpFiles.length >= 16,
    'all sixteen runtime images must have WebP outputs'
  )
  const total = webpFiles.reduce(
    (bytes, path) => bytes + statSync(path).size,
    0
  )
...
test('runtime image tree contains no PNG files', () => {
  const pngFiles = walk(assetRoot).filter(
...
test('visual sections are server components', () => {
  for (const file of staticSections) {
    ...
    assert.doesNotMatch(
      text,
      /^["']use client["'];?/m,
      `${file} is still a Client Component`
    )
